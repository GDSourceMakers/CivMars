...
public class Furnance : Building, IRegystratabe, IBuildable, IInventory, IHasGui, ICrafter
{
	...
	public Stack craftingQueue = new Stack(5);
	...
	void Update()
	{
		...
		{
			CraftingProcess a = (CraftingProcess)craftingQueue.Peek();
			if (a.hasItems)
			{
	...
	{
		//throw new NotImplementedException();
		craftingQueue.Push(new CraftingProcess(GameRegystry.recepies[ID][i]));
		Debug.Log("asdasd");
	}
